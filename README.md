...
| ➕ **自定义** | 你来定义 | 通过 ➕ 按钮创建，自定义角色、提示词、模型 |

### ➕ 自定义 Agent 创建 & 管理

点击顶部状态栏的 ➕ 按钮即可创建自定义 Agent，支持模板快速填充、颜色/房间选择。创建后自动打开配置面板设置提示词和模型。用户创建的 Agent 支持删除（二次确认），内置 Agent 受保护。
...
你只需要自备 LLM API Key（支持阿里云百炼/OpenAI/DeepSeek 等 OpenAI 兼容接口）。
